# -*- coding: utf-8 -*-
"""
KPI 대량 확충 생성기 (KPI-019 ~ KPI-200, +182)
14개 구조 패밀리 × 색(6) × 카드수(2~5) 스윕.
- 파일당 13슬라이드(≤25), 패밀리별 1파일로 분산.
- 다중도형은 c.group_asset 으로 asset:<ID> 자체완결 그룹 (id_caption/title 제외).
...
# 색 스윕: c.C 명명키만 사용 (raw hex 금지). navy_600/navy_800은 role 미매핑이라 c.C 사용.
ANAMES = ["blue_500", "teal_500", "cyan_500", "purple_600", "navy_600", "navy_800"]
ACCENTS = [c.C[n] for n in ANAMES]

# 제안서 더미 지표 (5개까지 슬라이스)
DATA = [
    {"value": "1,000", "unit": "건",  "label": "검증완료 기업정보", "pct": 95, "prev": "820", "cur": "1,000", "delta": "+22%"},
    {"value": "40",    "unit": "개사", "label": "참여기업",         "pct": 82, "prev": "28",  "cur": "40",    "delta": "+43%"},
    {"value": "42",    "unit": "만$",  "label": "수출상담액",       "pct": 68, "prev": "33",  "cur": "42",    "delta": "+27%"},
    {"value": "4.6",   "unit": "점",   "label": "이용자 만족도",     "pct": 88, "prev": "4.2", "cur": "4.6",   "delta": "+10%"},
    {"value": "95",    "unit": "%",    "label": "목표 달성률",       "pct": 73, "prev": "84",  "cur": "95",    "delta": "+13%"},
]
GROUP_TITLE = "핵심 성과 요약"

# 카드수별 사이즈 맵
NSZ = {2: 46, 3: 40, 4: 34, 5: 28}      # 카드 빅넘버
LSZ = {2: 16, 3: 15, 4: 14, 5: 12}      # 카드 라벨
RINGNSZ = {2: 34, 3: 30, 4: 26, 5: 22}  # 링/게이지 % 숫자
GD = {2: 2.6, 3: 2.3, 4: 1.95, 5: 1.6}  # 링/게이지 지름
GGAP = {2: 1.0, 3: 0.8, 4: 0.6, 5: 0.45}
BADGEGD = {2: 2.0, 3: 1.8, 4: 1.5, 5: 1.25}
BADGESZ = {2: 34, 3: 30, 4: 24, 5: 20}
...
def hx(n, w, gap):
    """n개 요소를 폭 w·간격 gap 으로 슬라이드 가로중앙 정렬한 x좌표 리스트."""
    tot = n * w + (n - 1) * gap
    x0 = (SW - tot) / 2
...
def meta_default(n): return [{"value": d["value"], "unit": d["unit"], "label": d["label"]} for d in DATA[:n]]
def meta_pct(n):     return [{"value": str(d["pct"]), "unit": "%", "label": d["label"]} for d in DATA[:n]]
def meta_yoy(n):     return [{"value": d["cur"], "unit": d["unit"], "label": d["label"], "prev": d["prev"]} for d in DATA[:n]]
def meta_trend(n):   return [{"value": d["value"], "unit": d["unit"], "label": d["label"], "delta": d["delta"]} for d in DATA[:n]]


# ============================================================
# 패밀리 렌더 함수 — 각기 (A: 그룹대상 도형리스트, meta) 반환
# ============================================================
def render_filled(s, accent, n):
    A = []; gap = 0.4; cw = (11.5 - (n - 1) * gap) / n; ch = 2.8; y = 2.1
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
        ...
        A.append(sn(s, x, y + 0.55, cw, cd["value"], cd["unit"], num_color=WHITE, unit_color=GRAY_100,
                    num_size=NSZ[n], unit_size=15))
        A.append(c.add_text(s, x + 0.1, y + ch - 0.85, cw - 0.2, 0.7, cd["label"], size=LSZ[n], bold=True,
                            color=GRAY_100, align=CENTER))
    return A, meta_default(n)


def render_border(s, accent, n):
    A = []; gap = 0.4; cw = (11.5 - (n - 1) * gap) / n; ch = 2.8; y = 2.1
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
...
def render_topbar(s, accent, n):
    ...
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
        ...
        A.append(c.add_text(s, x, y, cw, 0.62, cd["label"], size=LSZ[n], bold=True, color=WHITE, align=CENTER))
        A.append(sn(s, x, y + 1.05, cw, cd["value"], cd["unit"], num_color=accent, unit_color=c.role("muted_text"),
                    num_size=NSZ[n], unit_size=15, h=1.2))
    return A, meta_default(n)


def render_iconnum(s, accent, n):
    A = []; gap = 0.4; cw = (11.5 - (n - 1) * gap) / n; ch = 3.0; y = 2.0
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
        ...
        A.append(sn(s, x, y + 0.35 + icd, cw, cd["value"], cd["unit"], num_color=accent,
                    unit_color=c.role("muted_text"), num_size=min(40, NSZ[n] + 2), unit_size=14, h=0.85))
        A.append(c.add_text(s, x + 0.1, y + ch - 0.65, cw - 0.2, 0.55, cd["label"], size=LSZ[n], bold=True,
                            color=c.role("body_text"), align=CENTER))
    return A, meta_default(n)


def render_yoy(s, accent, n):
    A = []; gap = 0.4; cw = (11.5 - (n - 1) * gap) / n; ch = 3.6; y = 1.7
    up = c.role("accent_secondary")
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
        card = c.add_box(s, x, y, cw, ch, fill=c.role("row_base"), line=c.role("border"), line_w=1.2, shape=RR)
        card.adjustments[0] = RADIUS; A.append(card)
        bar = c.add_box(s, x, y, cw, 0.52, fill=accent, shape=RR); bar.adjustments[0] = RADIUS; A.append(bar)
        A.append(c.add_text(s, x, y, cw, 0.52, cd["label"], size=LSZ[n], bold=True, color=WHITE, align=CENTER))
        A.append(c.add_text(s, x + 0.1, y + 0.72, cw - 0.2, 0.35, "전년 %s%s" % (cd["prev"], cd["unit"]),
                            size=12, bold=True, color=c.role("muted_text"), align=CENTER))
        A.append(c.add_text(s, x, y + 1.12, cw, 0.4, UP_MARK, size=18, bold=True, color=up, align=CENTER))
        A.append(sn(s, x, y + 1.6, cw, cd["cur"], cd["unit"], num_color=c.role("header_fill"), unit_color=accent,
                    num_size=NSZ[n], unit_size=15))
        A.append(c.add_text(s, x, y + ch - 0.6, cw, 0.45, "%s 전년比" % cd["delta"], size=13, bold=True,
                            color=up, align=CENTER))
    return A, meta_yoy(n)


def render_ring(s, accent, n):
    A = []; gd = GD[n]; gy = 2.2; ggap = GGAP[n]
    for x, cd in zip(hx(n, gd, ggap), DATA[:n]):
        frac = cd["pct"] / 100.0
        ...
        A.append(sn(s, x, gy + gd / 2 - 0.4, gd, str(cd["pct"]), "%", num_color=c.role("header_fill"),
                    unit_color=accent, num_size=RINGNSZ[n], unit_size=13))
        A.append(c.add_text(s, x - 0.2, gy + gd + 0.1, gd + 0.4, 0.5, cd["label"], size=13, bold=True,
                            color=c.role("body_text"), align=CENTER))
    return A, meta_pct(n)


def render_gauge(s, accent, n):
    A = []; gd = GD[n]; gy = 2.4; ggap = GGAP[n]
    for x, cd in zip(hx(n, gd, ggap), DATA[:n]):
        ...
        A.append(sn(s, x, cyl - 0.68, gd, str(cd["pct"]), "%", num_color=c.role("header_fill"),
                    unit_color=accent, num_size=RINGNSZ[n], unit_size=13))
        A.append(c.add_text(s, x - 0.2, cyl + 0.12, gd + 0.4, 0.5, cd["label"], size=13, bold=True,
                            color=c.role("body_text"), align=CENTER))
    return A, meta_pct(n)


def render_badge(s, accent, n):
    A = []; bd = BADGEGD[n]; by = 2.4; bgap = GGAP[n]
    for x, cd in zip(hx(n, bd, bgap), DATA[:n]):
        ...
        c.set_shape_text(badge, cd["value"], size=BADGESZ[n], bold=True, color=WHITE); A.append(badge)
        A.append(c.add_text(s, x - 0.2, by + bd + 0.15, bd + 0.4, 0.5, cd["label"], size=13, bold=True,
                            color=c.role("body_text"), align=CENTER))
    return A, meta_default(n)


def render_hero(s, accent, n):
    ...
    A.append(sn(s, lx + 0.2, y + 0.7, lw - 0.4, d0["value"], d0["unit"], num_color=WHITE, unit_color=GRAY_100,
                num_size=72, unit_size=24, h=1.6))
    A.append(c.add_text(s, lx + 0.3, y + H - 1.3, lw - 0.6, 1.0, d0["label"], size=18, bold=True,
                        color=GRAY_100, align=CENTER))
    rest = DATA[1:n]; rx = lx + lw + 0.4; rw = 11.0 - lw - 0.4; m = len(rest)
    if m > 0:
        ...
        for j, cd in enumerate(rest):
            ...
            A.append(sn(s, rx + 0.4, yy, rw * 0.45, cd["value"], cd["unit"], num_color=accent,
                        unit_color=c.role("muted_text"), num_size=24, unit_size=12, align=LEFT, h=rh))
            A.append(c.add_text(s, rx + rw * 0.45 + 0.5, yy, rw * 0.55 - 0.6, rh, cd["label"], size=13, bold=True,
                                color=c.role("body_text"), align=LEFT))
    return A, meta_default(n)


def render_strip(s, accent, n):
    A = []; rw = 11.0; rx = (SW - rw) / 2; top = 1.7; bottom = 6.1; H = bottom - top
    rgap = 0.22; rh = (H - (n - 1) * rgap) / n
    for j, cd in enumerate(DATA[:n]):
        yy = top + j * (rh + rgap)
        ...
        A.append(c.add_text(s, rx + 0.5, yy, rw * 0.6, rh, cd["label"], size=16, bold=True,
                            color=c.role("body_text"), align=LEFT))
        A.append(sn(s, rx + rw * 0.6, yy, rw * 0.4 - 0.35, cd["value"], cd["unit"], num_color=accent,
                    unit_color=c.role("muted_text"), num_size={2: 34, 3: 30, 4: 26, 5: 22}[n], unit_size=13,
                    align=RIGHT, h=rh))
    return A, meta_default(n)


def render_trend(s, accent, n):
    A = []; gap = 0.4; cw = (11.5 - (n - 1) * gap) / n; ch = 3.3; y = 1.8
    ...
    for x, cd in zip(hx(n, cw, gap), DATA[:n]):
        ...
        A.append(c.add_text(s, x + 0.2, y + 0.25, cw - 0.4, 0.4, cd["label"], size=LSZ[n], bold=True,
                            color=c.role("muted_text"), align=LEFT))
        A.append(sn(s, x + 0.2, y + 0.65, cw - 0.4, cd["value"], cd["unit"], num_color=c.role("header_fill"),
                    unit_color=c.role("muted_text"), num_size={2: 40, 3: 36, 4: 30, 5: 26}[n], unit_size=14, align=LEFT))
        A.append(c.add_text(s, x + 0.2, y + 1.5, cw - 0.4, 0.35, "%s %s 전년比" % (UP_MARK, cd["delta"]),
                            size=12, bold=True, color=c.role("accent_secondary"), align=LEFT))
    ...
    return A, meta_trend(n)


def render_quad(s, accent, n):
    A = []; cols = 2; rows = math.ceil(n / 2); pw = 9.0; px = (SW - pw) / 2; py = 1.9
    cellgap = 0.3; cw = (pw - cellgap * (cols - 1)) / cols; chh = 1.5
    for idx, cd in enumerate(DATA[:n]):
        ...
        A.append(sn(s, x + 0.3, yy + 0.28, cw - 0.6, cd["value"], cd["unit"], num_color=accent,
                    unit_color=c.role("muted_text"), num_size=32, unit_size=14, align=LEFT, h=0.8))
        A.append(c.add_text(s, x + 0.3, yy + chh - 0.55, cw - 0.6, 0.45, cd["label"], size=13, bold=True,
                            color=c.role("muted_text"), align=LEFT))
    return A, meta_default(n)


def render_grouplabel(s, accent, n):
    A = []; cw = 11.0; cx = (SW - cw) / 2; y = 2.0
    hb = c.add_box(s, cx, y, cw, 0.7, fill=accent, shape=RR); hb.adjustments[0] = 0.12; A.append(hb)
    A.append(c.add_text(s, cx + 0.3, y, cw - 0.6, 0.7, GROUP_TITLE, size=16, bold=True, color=WHITE, align=LEFT))
    ch = 2.6; cy = y + 0.85
    ...
    for i, cd in enumerate(DATA[:n]):
        ...
        A.append(sn(s, sx, cy + 0.55, seg, cd["value"], cd["unit"], num_color=accent, unit_color=c.role("muted_text"),
                    num_size={2: 40, 3: 34, 4: 30, 5: 26}[n], unit_size=14))
        A.append(c.add_text(s, sx, cy + ch - 0.8, seg, 0.5, cd["label"], size=13, bold=True,
                            color=c.role("body_text"), align=CENTER))
    return A, meta_default(n)


def render_band(s, accent, n):
    A = []; bw = 12.0; bh = 2.0; bx = (SW - bw) / 2; byy = 2.7
    ...
    for i, cd in enumerate(DATA[:n]):
        ...
        A.append(sn(s, sx, byy + 0.45, seg, cd["value"], cd["unit"], num_color=WHITE, unit_color=GRAY_300,
                    num_size={2: 40, 3: 34, 4: 30, 5: 26}[n], unit_size=15))
        A.append(c.add_text(s, sx, byy + bh - 0.6, seg, 0.45, cd["label"], size=13, bold=True,
                            color=GRAY_100, align=CENTER))
    return A, meta_default(n)
...
FAMILIES = [
    ...
    ("topbar", "상단 컬러바 카드", "topbar-card", render_topbar,
     ["상단컬러바", "헤더바", "카드세트"], [], ["항목별성과", "핵심지표", "실적강조"]),
    ...
    ("ring", "링 프로그레스", "ring-progress", render_ring,
     ["링", "도넛", "프로그레스", "비율강조"], ["pct"], ["달성률", "목표대비", "진행률"]),
    ...
    ("band", "통계 밴드", "stat-band", render_band,
     ["통계밴드", "가로띠", "구분선", "지표"], [], ["종합성과", "핵심지표", "한눈요약"]),
]

CARD_COUNTS = [2, 3, 4, 5]
PER_FAMILY = 13  # 14 * 13 = 182 == KPI-019..200
...
def main():
    ...
    for f, (slug, kr, style, render, tags_x, edit_x, ruse) in enumerate(FAMILIES):
        prs = c.new_deck()
        file_rel = "decks/02_kpi/KPI_bulk_%s_v1.pptx" % slug
        for k in range(PER_FAMILY):
            ...
            title(s, "%s · %d카드 · %s" % (kr, n, aname))
            A, meta = render(s, accent, n)
            c.group_asset(s, A, aid)
            allentries.append(c.entry(
                asset_id=aid, category="KPI", name="%s %d종 (%s)" % (kr, n, aname),
                file_rel=file_rel, slide_idx=k + 1,
                tags=["KPI", "성과지표", kr] + tags_x,
                params={"style": style, "count": n, "accent": aname, "family": slug, "num_size": NSZ[n]},
    ...
    frag = c.write_fragment("KPI_bulk", allentries)
    print("FILES:")
    for p in files:
        print("  ", p)
    print("FRAG:", frag)
