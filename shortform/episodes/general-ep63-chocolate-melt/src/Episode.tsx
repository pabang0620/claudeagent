...
import {
  S1Bite, S2Question, S3Lattice, S4Thermo, S5Melt, S6Summer, S7Compare,
} from './scenes';
...
/** s1(초콜릿 한 조각을 입에 넣자마자 순식간에 녹는 모습, 무성)은 발화가 없어 잴 대상이 없다.
 *  대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "어? 손에 있을 땐 멀쩡했는데...") -> s3(설명 시작) 전환만 원칙 4에
...
/** s1 무성 구간 - 초콜릿이 입에 닿는 순간(bite 재사용, "음식·씹기 소재 전반 재사용 가능"으로
 *  등록된 소리를 초콜릿이 입에 들어가는 순간에 붙인다). */
const S1_BITE_SFX_AT = 0.32;
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Bite as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
    ...
    {
      Component: S4Thermo as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Melt as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Summer as unknown as SceneSpec['Component'], frames: frames[5],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 초콜릿이 입에 닿는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_BITE_SFX_AT)}
          durationInFrames={BITE_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/bite.mp3')} volume={0.7} />
        </Sequence>
      </Sequence>
