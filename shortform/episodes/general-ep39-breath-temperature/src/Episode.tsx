...
import {
  S1Rub, S1_RUB_SFX_AT, S2Question, S3Warm, S4Cool, S5Compare, S6Fan, S6_FAN_SFX_AT_FRAME,
} from './scenes';
 ...
 *  3인칭 설명 내레이션이 다이어그램/장면 위에 흐르는 구간이라 캐릭터 립싱크가 없다
 *  (ep06/ep22/ep24/ep33/ep35/ep37 등과 동일 원칙). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(손이 시려 두 손을 비비다 입 가까이 가져가는 무성 구간)은 발화가 없어 잴 대상이
 *  없다. 대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "어? 하~는 따뜻한데... 왜 이러는 거지?") -> s3(설명 시작) 전환만
...
const S1_RUB_SFX_FRAMES = 9; // hand_rub.mp3 실측 0.30초(30fps 9프레임)
const S6_FAN_SFX_FRAMES = 11; // fan_whir.mp3 실측 0.35초(30fps 10.5프레임, 반올림)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    ...
    {
      Component: S3Warm as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Cool as unknown as SceneSpec['Component'], frames: frames[3],
    ...
    {
      Component: S6Fan as unknown as SceneSpec['Component'], frames: frames[5],
    ...
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        <Sequence
          ...
          durationInFrames={S1_RUB_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/hand_rub.mp3')} volume={0.8} />
        </Sequence>
        {/* s6: 선풍기 스위치를 켜는 순간 */}
        <Sequence
          from={starts[5] + S6_FAN_SFX_AT_FRAME}
          durationInFrames={S6_FAN_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/fan_whir.mp3')} volume={0.75} />
        </Sequence>
      </Sequence>
