 ...
 *
 *  s1(벽 옆에서 머리를 기울여 귀를 벽에 가져다 대는 동작)은 무성 구간이라 발화 길이가 없다 -
 *  general-ep01의 S1Bite/general-ep87의 S1Clip과 같은 원칙으로 대본이 지정한 고정 길이
 *  (SILENT_DURATION_S1=2.0초, 02-script-v1.md s1 구간 추정)를 그대로 쓴다.
 *
 *  s2("어? 벽 너머인데 왜 이렇게 소리가 다 들리지?" - 리액션+훅 질문) 다음이 곧바로
 *  s3(설명 - 소리는 공기가 떨리는 것)으로 이어지는 전환이라 원칙 4에 따라 기본 여백(0.2초)
 *  보다 길게 준다(S2_TO_S3_PAD=0.6초, general-ep87·ep84와 같은 이유). s1->s2 전환은 무성
 *  동작에서 리액션으로 바로 이어지는 연속 동작이라 추가 여백 없이 0으로 둔다.
 *
 *  원칙 7(무성 구간·핵심 액션 효과음): s1(귀가 벽에 닿는 순간에 wall_thump - 벽 너머로
 *  어렴풋이 들려오는 낮은 "쿵쿵" 저음, REGISTRY 7절 신규 등록)에 짧은 효과음을 붙인다.
 *  내레이션(volume=1.6)보다 낮은 0.75로 보조음 수준을 유지한다.
 */
...
import {
  S1Listen, S1_CONTACT_AT_FRAME,
  S2React, S3Emit, S4Vibrate, S5Transmit, S6FreqCompare, S7Beat,
} from './scenes';
...
/** s1: 귀가 벽에 닿는 순간 wall_thump. scenes.tsx의 로컬 프레임 상수를 그대로 가져다 쓴다 -
 *  여기서 다시 추정하지 않는다 */
const WALL_THUMP_FRAMES = 38; // wall_thump.mp3 실측 1.28초(30fps 약 38프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Listen as unknown as SceneSpec['Component'], frames: frames[0] },
    {
      Component: S2React as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, frames: frames[1], mouth },
    },
    {
      Component: S3Emit as unknown as SceneSpec['Component'], frames: frames[2],
      props: { lines: linesS3, frames: frames[2] },
    },
    {
      Component: S4Vibrate as unknown as SceneSpec['Component'], frames: frames[3],
      props: { lines: linesS4, frames: frames[3] },
    },
    {
      Component: S5Transmit as unknown as SceneSpec['Component'], frames: frames[4],
      props: { lines: linesS5, frames: frames[4] },
    },
    {
      Component: S6FreqCompare as unknown as SceneSpec['Component'], frames: frames[5],
      props: { lines: linesS6, frames: frames[5] },
    },
    {
      Component: S7Beat as unknown as SceneSpec['Component'], frames: frames[6],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 귀가 벽에 닿는 순간 */}
        <Sequence from={starts[0] + S1_CONTACT_AT_FRAME} durationInFrames={WALL_THUMP_FRAMES} layout="none">
          <Audio src={staticFile('audio/wall_thump.mp3')} volume={0.75} />
        </Sequence>
      </Sequence>
