 ...
 *
 *  s1(술잔을 들어 한 모금 마심)은 무성 동작 구간이라 발화 길이가 없다 - general-ep01의
 *  S1Bite와 같은 원칙으로 대본이 지정한 고정 길이(SILENT_DURATION_S1)를 그대로 쓴다.
 *  s2는 캐릭터가 직접 대사를 말하는 바스트샷이라 mouth.json으로 립싱크한다.
 */
...
import {
  S1Sip, S1_SIP_PEAK_LOCAL, S2React, S3Convert, S4Enzyme, S5Flush, S6Compare, S7Outro,
} from './scenes';
...
/** s1(술잔 들어 한 모금)은 무성 - 발화 길이로 잴 대상이 없어 대본이 지정한 동작 길이를 쓴다 */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2("어, 나만 왜 이렇게 얼굴이 빨개지지" - 리액션+훅 질문) 다음이 곧바로 설명(s3)으로
 *  넘어가는 전환이라 원칙 4에 따라 기본 여백보다 길게 준다(general-ep01의 S2_TO_S3_PAD와
 *  같은 이유 - 질문이 숨 쉴 틈 없이 바로 설명이 붙으면 궁금해할 틈이 없다는 지적). 다른
 *  전환(s1->s2, s3->s4, ...)은 프로필 기본 여백(NARRATED_PAD=0.2s) 그대로 둔다. */
const S2_TO_S3_PAD = 0.6;
const SCENE_PAD = [0, S2_TO_S3_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD];

/** sip_slurp SFX. s1 은 언어 무관 고정 길이라 starts[0]=0 이 항상 성립한다.
 *  S1_SIP_PEAK_LOCAL(scenes.tsx) = 잔이 입에 닿는 순간(빌드 애니메이션 종료 프레임). */
const SIP_SFX_FRAMES = 10; // sip_slurp.mp3 실측 0.30초(30fps 9프레임) + 여유
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    ...
    {
      Component: S3Convert as unknown as SceneSpec['Component'], frames: frames[2],
      props: { lines: linesS3, frames: frames[2] },
    },
    {
      Component: S4Enzyme as unknown as SceneSpec['Component'], frames: frames[3],
      props: { lines: linesS4, frames: frames[3] },
    },
    {
      Component: S5Flush as unknown as SceneSpec['Component'], frames: frames[4],
    ...
    {
      Component: S7Outro as unknown as SceneSpec['Component'], frames: frames[6],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 잔이 입에 닿는 순간 - "훌쩍" 한 모금 소리. 언어 무관 공용 자산 */}
        <Sequence from={starts[0] + S1_SIP_PEAK_LOCAL} durationInFrames={SIP_SFX_FRAMES} layout="none">
          <Audio src={staticFile('audio/sip_slurp.mp3')} volume={0.9} />
        </Sequence>
      </Sequence>
