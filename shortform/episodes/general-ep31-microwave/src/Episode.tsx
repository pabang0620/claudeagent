...
import {
  S1Start, S2Question, S3Wave, S4Friction, S5Compare, S6History,
} from './scenes';
 ...
 *  (ep06/ep22/ep24/ep27 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(음식이 놓이고 문이 닫히며 시작 버튼이 눌리는 무성 구간)은 발화가 없어 잴 대상이
 *  없다. 대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "근데 왜 이렇게 빨리 데워지지?") -> s3(설명 시작) 전환만 원칙 4에
...
/** s1 무성 구간 - 시작 버튼을 누르는 순간에 짧은 "톡" 클릭음(ui_tap 재사용, 원칙 7) */
const S1_BTN_SFX_AT = 0.51; // S1Start의 btnPressP 시작 지점(0.46)에 여유를 더한 프레임 비율
const TAP_SFX_FRAMES = 6; // ui_tap.mp3 실측 0.10초(30fps 3프레임) + 여유
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Start as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Wave as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Friction as unknown as SceneSpec['Component'], frames: frames[3],
    ...
    {
      Component: S6History as unknown as SceneSpec['Component'], frames: frames[5],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 시작 버튼을 누르는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_BTN_SFX_AT)}
