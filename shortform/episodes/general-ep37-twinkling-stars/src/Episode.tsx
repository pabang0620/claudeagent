...
import {
  S1Point, S2Question, S3PointZoom, S4Beam, S5Jitter, S6Twinkle, S7Disk, S8Average,
} from './scenes';
 ...
 *  (ep06/ep22/ep24/ep33/ep35 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(밤하늘 별 하나를 가리키는 무성 구간)은 발화가 없어 잴 대상이 없다. 대본이 지정한
 *  추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "저 별 좀 봐... 왜 저러지?") -> s3(설명 시작) 전환만 원칙 4에 따라
 *  여백을 늘린다. 다른 전환은 프로필 기본 여백(0.2s)을 그대로 쓴다. */
const S2_TO_S3_PAD = 0.6;

/** allSegments = [s1, s2, s3, s4, s5, s6, s7, s8]. pad[i]는 구간 i "끝"에 붙는 여백이다. */
const SCENE_PAD = [
  NARRATED_PAD, S2_TO_S3_PAD, NARRATED_PAD, NARRATED_PAD,
  NARRATED_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD,
];

const NARRATED_IDS = ['s2', 's3', 's4', 's5', 's6', 's7', 's8'];

/** s1 무성 구간 - 캐릭터가 별 하나를 가리키다 "어? 유독 반짝이네" 하고 알아채는 순간의
 *  발견 리액션 SFX(원칙 7, realize_ding 재사용 - "눈을 뜨거나 화면을 보다가 어? 하고
 *  알아채는 발견·자각 리액션 전반 재사용 가능"이라는 REGISTRY 설명과 정확히 일치해
 *  신규 합성 없이 기존 SFX를 그대로 썼다). 캐릭터가 손을 다 든 시점(scenes.tsx S1Point의
 *  pointT가 1에 도달하는 지점)에 맞춘다. */
const S1_REALIZE_SFX_AT = 0.55; // s1 프레임 수 기준 비율
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Point as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3PointZoom as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Beam as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Jitter as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Twinkle as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7Disk as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8Average as unknown as SceneSpec['Component'], frames: frames[7],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 캐릭터가 유독 반짝이는 별을 알아채는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_REALIZE_SFX_AT)}
          durationInFrames={REALIZE_SFX_FRAMES} layout="none"
        >
