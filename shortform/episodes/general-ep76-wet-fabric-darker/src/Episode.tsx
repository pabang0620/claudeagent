...
import {
  S1Hook, S2Dry, S3Wetting, S4Absorb, S5Compare, S6Montage, S7Dry, s1SplashFrame,
} from './scenes';
...
/** water_splash 효과음: s1에서 물방울이 셔츠에 닿는 프레임(scenes.tsx의 s1SplashFrame)에
 *  정확히 맞춰 재생(원칙 7). s1은 언어별로 길이가 달라 splash 프레임도 언어별로 다시
 *  계산한다(ep18과 동일 패턴). */
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 물방울이 셔츠에 닿는 순간 - "톡" 물소리(원칙 7, REGISTRY의 water_splash.mp3
         *  재사용 - 물이 어딘가에 닿는 동작 전반) */}
        <Sequence from={starts[0] + s1SplashFrame(frames[0])} durationInFrames={WATER_SFX_FRAMES} layout="none">
