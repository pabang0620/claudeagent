...
import {
  S1_TAP1_AT, S1_TAP2_AT, S1TapWall, S2Question, S3ForkDiagram, S4GlassPass, S5GlassFinal,
  S6WallScatter, S7WallFinal, S8FrostedGlass,
} from './scenes';
...
/** s1(유리창 너머로 풍경을 보다가 옆 벽을 톡톡 두드리는 무성 구간)은 발화가 없어 잴 대상이
 *  없다. 대본이 지정한 추정 길이(3.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 3.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "어? 유리는...왜 하나도 안 보이지?") -> s3(설명 시작) 전환만 원칙 4에
...
/** s1: 옆 벽을 "톡톡" 두드리는 무성 동작 - ui_tap 재사용("화면 UI 탭·버튼 누름 동작 전반
 *  재사용 가능"으로 등록돼 있으나, 벽을 짧게 두 번 두드리는 톡톡 소리로도 그대로 맞는다). */
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1TapWall as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3ForkDiagram as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4GlassPass as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5GlassFinal as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6WallScatter as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7WallFinal as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8FrostedGlass as unknown as SceneSpec['Component'], frames: frames[7],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 벽을 톡톡 두드리는 두 번의 탭 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_TAP1_AT)}
          durationInFrames={TAP_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/ui_tap.mp3')} volume={0.75} />
        </Sequence>
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_TAP2_AT)}
          durationInFrames={TAP_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/ui_tap.mp3')} volume={0.75} />
        </Sequence>
