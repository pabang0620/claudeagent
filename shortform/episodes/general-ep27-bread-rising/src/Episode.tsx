...
import {
  S1Rise, S2Question, S3Yeast, S4Net, S5Balloon, S6Bake, S7History,
} from './scenes';
 ...
 *  (ep06/ep22/ep24 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(볼 안 반죽 덩어리가 시간이 지나며 두 배로 부풀어 오르는 타임랩스)은 내레이션이 없는
 *  순수 동작 구간이라 발화 길이로 잴 대상이 없다. 대본이 지정한 길이(3.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 3.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "이 안에 뭐가 들어있길래 이러지?") -> s3(설명 시작) 전환만 원칙 4에
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Rise as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Yeast as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Net as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Balloon as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Bake as unknown as SceneSpec['Component'], frames: frames[5],
