...
import {
  S1Rub, S2Question, S3Light, S4Pressure, S5Brain, S6Sparkle, S7Warning,
} from './scenes';
...
/** s1(피곤해서 눈을 비비는 모습, 손을 떼자 반짝임이 잠깐 오버레이되는 무성 구간)은 발화가
 *  없어 잴 대상이 없다. 대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "어? 방금 눈에서 뭔가 반짝였는데?") -> s3(설명 시작) 전환만 원칙 4에
...
/** s1 무성 구간 - 손으로 눈을 비비는 동작(hand_rub 재사용, "손을 비비거나 문지르는 무성
 *  동작 전반 재사용 가능") + 손을 떼는 순간 반짝임을 알아채는 인지 신호음(realize_ding
 *  재사용). */
const S1_RUB_SFX_AT = 0.15;
const RUB_SFX_FRAMES = 9; // hand_rub.mp3 실측 0.30초(30fps 9프레임)
const S1_SPARKLE_SFX_AT = 0.62;
const SPARKLE_SFX_FRAMES = 9; // realize_ding.mp3 실측 0.30초(30fps 9프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Rub as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Light as unknown as SceneSpec['Component'], frames: frames[2],
    ...
    {
      Component: S5Brain as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Sparkle as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7Warning as unknown as SceneSpec['Component'], frames: frames[6],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 눈을 비비는 손 동작 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_RUB_SFX_AT)}
          durationInFrames={RUB_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/hand_rub.mp3')} volume={0.7} />
        </Sequence>
        {/* s1: 손을 떼며 반짝임을 알아채는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_SPARKLE_SFX_AT)}
          durationInFrames={SPARKLE_SFX_FRAMES} layout="none"
        >
