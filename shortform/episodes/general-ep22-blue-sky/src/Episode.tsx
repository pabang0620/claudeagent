...
import {
  S1LookUp, S2Question, S3Split, S4Scatter, S5Converge, S6SunsetShift, S7LongPath, S8VioletBlue,
} from './scenes';
...
/** s2(리액션+훅 질문)만 캐릭터가 직접 대사를 말하는 구간이라 mouth.json을 쓴다.
 *  s3~s8은 3인칭 설명 내레이션이 다이어그램/배경 위에 흐르는 구간이라 캐릭터가 등장하지
 *  않는다(원칙 - ep06/ep07 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(하늘을 올려다봄)은 내레이션이 없는 순수 동작 구간이라 발화 길이로 잴 대상이 없다.
 *  대본이 지정한 길이(2.0초)를 그대로 쓴다(다른 화의 SILENT_DURATION 패턴과 동일). */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "하늘은 왜 맨날 파란 색일까?") -> s3(설명 시작) 전환만 원칙 4에 따라
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1LookUp as unknown as SceneSpec['Component'], frames: frames[0], props: {} },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Split as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Scatter as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Converge as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6SunsetShift as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7LongPath as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8VioletBlue as unknown as SceneSpec['Component'], frames: frames[7],
