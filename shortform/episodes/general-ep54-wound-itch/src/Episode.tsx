...
import {
  S1Wound, S2Question, S3Heal, S4Histamine, S5NerveHit, S6Tighten, S7ToBrain, S8Restrain,
} from './scenes';
 ...
 *  s8은 캐릭터가 다시 등장하지만 대사가 아니라 결과를 설명하는 내레이터 음성이라
 *  mouth.json을 연결하지 않는다(ep33 s8과 같은 처리 - 원칙 5 예방 체크리스트는 "리액션
 *  대사가 있는 장면"만 대상이고, s8은 리액션 대사가 아니다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(무릎 상처 클로즈업, 손이 다가가다 멈칫하는 무성 구간)은 발화가 없어 잴 대상이 없다.
 *  대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "왜 이렇게 간지럽지?") -> s3(설명 시작) 전환만 원칙 4에 따라 여백을
...
/** s1 무성 구간 - 손이 상처 쪽으로 다가갔다가 멈칫하는 순간(scenes.tsx S1Wound의
 *  retreatT 시작점 = frames*0.62)에 "어, 참아야지" 하고 알아채는 인지 신호음(realize_ding
 *  재사용, 원칙 7 - REGISTRY 설명 "발견·자각 리액션 전반 재사용 가능"과 일치). */
const S1_PAUSE_SFX_AT = 0.6;
const PAUSE_SFX_FRAMES = 9; // realize_ding.mp3 실측 0.30초(30fps 9프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Wound as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Heal as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Histamine as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5NerveHit as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Tighten as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7ToBrain as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8Restrain as unknown as SceneSpec['Component'], frames: frames[7],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 손이 멈칫하며 물러나는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_PAUSE_SFX_AT)}
          durationInFrames={PAUSE_SFX_FRAMES} layout="none"
        >
