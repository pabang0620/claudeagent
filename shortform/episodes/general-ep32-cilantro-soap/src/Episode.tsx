...
import {
  S1TwoEaters, S1_BITE_SFX_FRAME, S2Reaction, S3Compare, S4Receptor, S5Verdict, S6Fading,
} from './scenes';
 ...
 *  (원칙 - ep24/ep28/ep30과 동일). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(무성, 두 캐릭터가 나란히 고수 음식을 먹는 도입)은 내레이션이 없는 순수 관찰 구간이라
 *  발화 길이로 잴 대상이 없다. 대본이 지정한 길이(3.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 3.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "근데 옆 사람은 맛있다는데?") -> s3(설명 시작) 전환만 원칙 4에 따라
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1TwoEaters as unknown as SceneSpec['Component'], frames: frames[0], props: {} },
    {
    ...
    {
      Component: S4Receptor as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Verdict as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5, leftQuote: t.s5LeftQuote, rightQuote: t.s5RightQuote },
    },
    {
      Component: S6Fading as unknown as SceneSpec['Component'], frames: frames[5],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 왼쪽 캐릭터가 한 입 크게 베어 무는 순간(원칙 7, 무성 구간 효과음) */}
        <Sequence from={starts[0] + S1_BITE_SFX_FRAME} durationInFrames={20} layout="none">
