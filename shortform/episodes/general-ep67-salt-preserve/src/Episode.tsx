/** 본편 조립. Intro + TitleCard + 7개 장면(전부 유성 내레이션) + Outro.
...
import {
  S1Food, S2Osmosis, S3BacteriaShrink, S4GrowthBlock, S5Compare, S6History, S7Concentration,
} from './scenes';
...
/** 이 화는 캐릭터 바스트샷 없이 다이어그램·카드·실루엣만으로 진행돼 립싱크 대상이 없다.
 *  public/audio/ko_mouth.json은 표준 파이프라인 산출물로 그대로 생성해뒀지만(원칙 2)
 *  씬에서 쓰지 않아 여기서 import하지 않는다. */
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Food as unknown as SceneSpec['Component'], frames: frames[0], props: { lines: linesS1 } },
    {
      Component: S2Osmosis as unknown as SceneSpec['Component'], frames: frames[1],
      props: { frames: frames[1], lines: linesS2 },
    },
    {
      Component: S3BacteriaShrink as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4GrowthBlock as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    { Component: S5Compare as unknown as SceneSpec['Component'], frames: frames[4], props: { lines: linesS5 } },
    { Component: S6History as unknown as SceneSpec['Component'], frames: frames[5], props: { lines: linesS6 } },
    {
      Component: S7Concentration as unknown as SceneSpec['Component'], frames: frames[6],
    ...
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {allSegments.map((seg, i) => (
          <Sequence key={seg.id} from={starts[i]} durationInFrames={frames[i]} layout="none">
            <Audio src={staticFile(`audio/${locale}_${seg.id}.mp3`)} volume={1.6} />
          </Sequence>
        ))}
      </Sequence>
