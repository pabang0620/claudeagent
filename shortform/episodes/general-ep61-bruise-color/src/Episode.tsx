...
import {
  S1Look, S2Question, S3Vessel, S4InitialColor, S5Cells, S6ToGreen, S7ToYellow, S8Order,
} from './scenes';
...
/** s1(팔에 이미 든 멍 자국을 들여다보는 모습, 무성)은 발화가 없어 잴 대상이 없다.
 *  대본이 지정한 추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "어, 근데 이 멍 색깔이 계속 바뀌네?") -> s3(설명 시작) 전환만 원칙 4에
...
/** s1 무성 구간 - 팔에 든 멍을 살펴보다가 손끝으로 살짝 눌러보는 동작(hand_rub 재사용,
 *  "손을 비비거나 문지르는 무성 동작 전반 재사용 가능"으로 등록된 소리를 살짝 누르는
 *  순간에 붙인다). */
const S1_PRESS_SFX_AT = 0.55;
const PRESS_SFX_FRAMES = 9; // hand_rub.mp3 실측 0.30초(30fps 9프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Look as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Vessel as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4InitialColor as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Cells as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6ToGreen as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7ToYellow as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8Order as unknown as SceneSpec['Component'], frames: frames[7],
