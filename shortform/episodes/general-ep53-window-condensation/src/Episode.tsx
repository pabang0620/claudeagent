 ...
 *
 *  s1만 캐릭터(Actor)가 등장해 립싱크(ko_mouth.json)를 쓴다. 나머지는 다이어그램·소품 위주
 *  설명 구간이라 mouth를 소비하지 않는다. 모든 구간이 담담한 설명 구간이고(리액션+훅 질문
 *  구간이 없다), 전환 여백 확대(원칙 4의 5번)가 필요한 지점이 없어 전 구간에 프로필 기본
 *  여백(0.2초)만 쓴다.
 */
...
import {
  S1Window, S2Vapor, S3Compare, S4Contact, S5Droplets, S6CrossSection, S7Cup,
} from './scenes';
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    {
      Component: S1Window as unknown as SceneSpec['Component'], frames: frames[0],
      props: { lines: linesS1, frames: frames[0], mouth },
    },
    {
      Component: S2Vapor as unknown as SceneSpec['Component'], frames: frames[1],
    ...
    {
      Component: S4Contact as unknown as SceneSpec['Component'], frames: frames[3],
      props: { lines: linesS4, frames: frames[3] },
    },
    {
      Component: S5Droplets as unknown as SceneSpec['Component'], frames: frames[4],
      props: { lines: linesS5, frames: frames[4] },
    },
    {
      Component: S6CrossSection as unknown as SceneSpec['Component'], frames: frames[5],
      props: { lines: linesS6, frames: frames[5] },
    },
    {
      Component: S7Cup as unknown as SceneSpec['Component'], frames: frames[6],
