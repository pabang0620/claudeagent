...
import {
  S1Taste, S2Question, S3Dissolve, S4Release, S5River, S6Evap, S7Accumulate, S8DeadSea,
} from './scenes';
...
/** s2(리액션+훅 질문)만 캐릭터가 직접 대사를 말하는 구간이라 mouth.json을 쓴다. s3~s8은
 *  3인칭 설명 내레이션이 다이어그램/장식 위에 흐르는 구간이라 캐릭터가 등장하지 않는다
 *  (ep06/ep22/ep24/ep27/ep31 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(바닷물을 손으로 떠서 맛보는 무성 구간)은 발화가 없어 잴 대상이 없다. 대본이 지정한
 *  추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "근데 강물은 안 짠데 바닷물은 왜 이렇게 짤까?") -> s3(설명 시작)
 *  전환만 원칙 4에 따라 여백을 늘린다. 다른 전환은 프로필 기본 여백(0.2s)을 그대로 쓴다. */
...
/** s1 무성 구간 - 손을 물에 담그는 순간 "촤르르" 물소리(water_splash 재사용, 원칙 7),
 *  손을 입으로 가져가 맛보는 순간 "훌쩍" 소리(sip_slurp 재사용). 둘 다 각 동작의 애니메이션
 *  정점 프레임(scenes.tsx S1Taste의 dipT/sipT 정점)에 정확히 맞춘다. */
const S1_SPLASH_SFX_AT = 16 / 60; // 손이 물에 닿는 순간(dip 정점, 60프레임 기준 비율)
const S1_SIP_SFX_AT = 46 / 60; // 손이 입에 닿는 순간(sip 정점)
const SPLASH_SFX_FRAMES = 11; // water_splash.mp3 실측 0.36초(30fps 약 11프레임)
const SIP_SFX_FRAMES = 9; // sip_slurp.mp3 실측 0.30초(30fps 약 9프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Taste as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
    ...
    {
      Component: S5River as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Evap as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7Accumulate as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7, prefix: t.s7Prefix, suffix: t.s7Suffix },
    },
    {
      Component: S8DeadSea as unknown as SceneSpec['Component'], frames: frames[7],
