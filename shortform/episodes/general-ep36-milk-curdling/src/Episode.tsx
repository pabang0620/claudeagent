...
import {
  S1Pour, S2Question, S3Acid, S4Curdle, S5Result, S6Lemon, S7Foods,
  S1_SPLASH_SFX_AT, S6_POP_SFX_AT,
} from './scenes';
...
/** s2(리액션+훅 질문 "왜 이렇게 됐지?")만 캐릭터가 직접 대사를 말하는 구간이라 mouth.json을
 *  쓴다. s3~s7은 3인칭 설명 내레이션이 다이어그램/장식 위에 흐르는 구간이라 립싱크를 쓰지
 *  않는다(ep06/ep22/ep24/ep27/ep31/ep33 등과 동일 원칙). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(냉장고에서 꺼낸 우유를 컵에 따르는데 덩어리져 나오는 무성 구간)은 발화가 없어 잴
 *  대상이 없다. 대본이 지정한 추정 길이(3.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 3.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "왜 이렇게 됐지?") -> s3(설명 시작) 전환만 원칙 4에 따라 여백을
...
/** s1 무성 구간 - 우유를 컵에 따르는 순간 "촤르르" 물소리(water_splash 재사용, 원칙 7).
 *  s6 유성 구간 - 레몬즙이 떨어져 우유가 순식간에 덩어리지는 순간 "톡" 소리(bubble_pop
 *  재사용, "막이 훅 꺼지는" 질감이 액체가 갑자기 상태를 바꾸는 순간과 잘 맞는다). 둘 다
 *  각 동작의 애니메이션 정점 프레임(scenes.tsx S1Pour의 pourT, S6Lemon의 curdle 시작
 *  프레임)에 맞춘다. */
const SPLASH_SFX_FRAMES = 11; // water_splash.mp3 실측 0.36초(30fps 약 11프레임)
const POP_SFX_FRAMES = 6; // bubble_pop.mp3 실측 0.18초(30fps 약 6프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Pour as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Acid as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Curdle as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5Result as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Lemon as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7Foods as unknown as SceneSpec['Component'], frames: frames[6],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 우유가 컵에 따라지는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_SPLASH_SFX_AT)}
          durationInFrames={SPLASH_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/water_splash.mp3')} volume={0.8} />
        </Sequence>
        {/* s6: 레몬즙 한 방울에 우유가 순식간에 덩어리지는 순간 */}
        <Sequence
          from={starts[5] + Math.round(frames[5] * S6_POP_SFX_AT)}
          durationInFrames={POP_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/bubble_pop.mp3')} volume={0.85} />
        </Sequence>
      </Sequence>
