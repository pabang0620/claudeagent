 ...
 *
 *  s1은 무성(밤에 자전거로 달림 - 가로수는 빠르게 스치는데 달은 같은 자리) - ep01/ep40의
 *  SILENT_DURATION 패턴을 그대로 따른다(대본이 정한 길이 0:00-2.00 = 2.0초를 그대로 쓴다.
 *  원칙 4 - 길이를 채우려고 늘리지 않는다). s2(리액션+훅 "달은 왜 계속 따라오지?")만 캐릭터
 *  본인의 대사라 립싱크를 연결한다. s3~s8은 3인칭 설명/디아그램 구간이라 립싱크를 쓰지 않는다.
 */
...
import {
  S1MoonRide, S2Question, S3NearAngle, S4PassingScenery, S5MoonDistance, S6TinyChange,
  S7MoonSteady, S8TrainCompare,
} from './scenes';
...
/** s2(리액션 "달은 왜 계속 따라오지?")만 캐릭터 본인이 직접 말하는 구간이라 mouth.json으로
 *  입을 움직인다. s3~s8은 다이어그램/무성 배경 재사용 장면이라 립싱크를 쓰지 않는다. */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(무성, 밤 자전거)은 발화 길이로 잴 대상이 없어 대본이 정한 길이(0:00-2.00)를 그대로
 *  초 단위로 쓴다(ep01/ep40의 SILENT_DURATION 패턴과 동일). */
const S1_DURATION_SEC = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 "달은 왜 계속 따라오지?") -> s3(설명 시작)로 넘어가는 전환만 여백을 늘린다
 ...
 *  여백(=다음 구간 시작 전 여백)이다. 8개 구간 전부에 값이 있어야 한다. */
const SCENE_PAD = [
  0, S2_TO_S3_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD, NARRATED_PAD,
  NARRATED_PAD,
];

/** head_whoosh SFX. s1(무성) 안에서 가로수가 휙휙 지나가는 느낌을 두 번 준다(원칙 7,
 *  "고개·시선을 빠르게 돌리는 무성 동작 전반 재사용 가능"으로 REGISTRY에 등록된 자산을
 *  "빠르게 스쳐 지나가는" 무성 동작 전반으로 재사용). s1이 언어 무관 고정 길이라
 *  starts[0](=0)은 항상 같은 프레임이다. */
const HEAD_WHOOSH_AT = [0.28, 0.66]; // s1 구간 길이 대비 비율
const HEAD_WHOOSH_SFX_FRAMES = 8; // head_whoosh.mp3 실측 0.24초(30fps 7.2프레임) + 여유

/** realize_ding SFX. s2가 "어"로 시작하는 발견의 순간(원칙 7)에 맞춘다 - ko_words.json 실측
 *  s2의 "어" 시작 0.088초(30fps 약 2.6프레임)에 맞춰 로컬 프레임 2에 건다. */
const REALIZE_DING_AT_FRAME = 2;
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1MoonRide as unknown as SceneSpec['Component'], frames: frames[0], props: {} },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { frames: frames[1], lines: linesS2, mouth },
    },
    {
      Component: S3NearAngle as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4PassingScenery as unknown as SceneSpec['Component'], frames: frames[3],
      props: { frames: frames[3], lines: linesS4 },
    },
    {
      Component: S5MoonDistance as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6TinyChange as unknown as SceneSpec['Component'], frames: frames[5],
      props: { frames: frames[5], lines: linesS6 },
    },
    {
      Component: S7MoonSteady as unknown as SceneSpec['Component'], frames: frames[6],
      props: { frames: frames[6], lines: linesS7 },
    },
    {
      Component: S8TrainCompare as unknown as SceneSpec['Component'], frames: frames[7],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 가로수가 휙휙 지나가는 무성 구간 */}
        {HEAD_WHOOSH_AT.map((ratio, i) => (
          <Sequence
            key={`whoosh-${i}`}
            from={starts[0] + Math.round(frames[0] * ratio)}
            durationInFrames={HEAD_WHOOSH_SFX_FRAMES} layout="none"
          >
            <Audio src={staticFile('audio/head_whoosh.mp3')} volume={0.6} />
          </Sequence>
        ))}
