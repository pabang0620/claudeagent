/** 본편 조립. Intro + TitleCard + 7개 장면(전부 유성 - 리액션 대사 없이 전부 3인칭 설명
 *  내레이션) + Outro.
 ...
 *
 *  이 화는 s1~s7 전부 3인칭 설명 내레이션이라(캐릭터가 자기 대사를 말하는 리액션 구간이
 *  없음) mouth.json/mouthAt을 연결하지 않는다 - ep54 s8과 같은 처리(원칙 5 예방 체크리스트는
 *  "리액션 대사가 있는 장면"만 대상).
 */
...
import {
  S1Balloon, S2Cords, S3Combine, S4Compare, S5TimbreUp, S6Contrast, S7Warning,
} from './scenes';
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    {
      Component: S1Balloon as unknown as SceneSpec['Component'], frames: frames[0],
      props: { frames: frames[0], lines: linesS1 },
    },
    {
      Component: S2Cords as unknown as SceneSpec['Component'], frames: frames[1],
      props: { frames: frames[1], lines: linesS2 },
    },
    {
      Component: S3Combine as unknown as SceneSpec['Component'], frames: frames[2],
    ...
    {
      Component: S5TimbreUp as unknown as SceneSpec['Component'], frames: frames[4],
      props: { frames: frames[4], lines: linesS5 },
    },
    {
      Component: S6Contrast as unknown as SceneSpec['Component'], frames: frames[5],
...
export const totalFramesFor = (locale: Locale) => {
  const allSegments = WORDS_BY_LANG[locale].segments;
  const frames = sceneFrames(allSegments, SCENE_PAD);
  const mainTotal = frames.reduce((a, b) => a + b, 0);
