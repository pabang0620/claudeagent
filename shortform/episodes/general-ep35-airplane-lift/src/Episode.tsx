...
import {
  S1Takeoff, S2Question, S3Deflect, S4Reaction, S5Conclusion, S6Simultaneity,
} from './scenes';
 ...
 *  (ep06/ep22/ep24/ep27/ep31/ep33 등과 동일하게 다이어그램 장면엔 립싱크를 쓰지 않는다). */
const MOUTH_BY_LANG: Record<Locale, MouthFile> = {
  ko: koMouthRaw as MouthFile,
};

/** s1(활주로에서 가속하다 이륙하는 무성 구간)은 발화가 없어 잴 대상이 없다. 대본이 지정한
 *  추정 길이(2.0초)를 그대로 쓴다. */
const SILENT_DURATION_S1 = 2.0;
const NARRATED_PAD = 0.2;
/** s2(리액션+훅 질문 "근데 저 무거운 게 어떻게 뜨는 거지?") -> s3(설명 시작) 전환만
...
/** s1 무성 구간 - 비행기가 이륙(climb)을 시작하는 순간 엔진 소리가 확 커지는 "engine_takeoff"
 *  SFX(원칙 7, 신규 합성). scenes.tsx S1Takeoff의 liftP 시작 지점(frames*0.52)에 맞춘다. */
const S1_ENGINE_SFX_AT = 0.5; // 60프레임 기준 비율(대략 liftP 시작 직전)
const ENGINE_SFX_FRAMES = 15; // engine_takeoff.mp3 실측 0.5초(30fps 15프레임)
...
export const Episode: React.FC<EpisodeProps> = ({ locale }) => {
  ...
  const scenes: SceneSpec[] = [
    { Component: S1Takeoff as unknown as SceneSpec['Component'], frames: frames[0], props: { frames: frames[0] } },
    {
      Component: S2Question as unknown as SceneSpec['Component'], frames: frames[1],
      props: { lines: linesS2, mouth },
    },
    {
      Component: S3Deflect as unknown as SceneSpec['Component'], frames: frames[2],
      props: { frames: frames[2], lines: linesS3 },
    },
    {
      Component: S4Reaction as unknown as SceneSpec['Component'], frames: frames[3],
    ...
    {
      Component: S6Simultaneity as unknown as SceneSpec['Component'], frames: frames[5],
  ...
  return (
    <AbsoluteFill style={{ background: C.paper }}>
      ...
      <Sequence from={INTRO_FRAMES + TITLE_CARD_FRAMES} durationInFrames={mainTotal} layout="none">
        ...
        {/* s1: 비행기가 이륙(climb)을 시작하는 순간 */}
        <Sequence
          from={starts[0] + Math.round(frames[0] * S1_ENGINE_SFX_AT)}
          durationInFrames={ENGINE_SFX_FRAMES} layout="none"
        >
          <Audio src={staticFile('audio/engine_takeoff.mp3')} volume={0.85} />
        </Sequence>
      </Sequence>
