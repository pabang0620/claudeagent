...
**위험 수준별 처리:**
- 위험 수준(안전/주의/위험)과 무관하게 삭제 실행 전 반드시 대상 목록을 사용자에게 제시하고 명시적 승인을 받는다 - 등급은 승인 요청 시 위험도 참고용으로만 표기하며, 어떤 등급도 승인 없이 자동 진행하지 않는다 ("다음 파일들을 제거해도 될까요? [등급: 안전/주의/위험]")
- 위험 등급 항목은 특히 신중히 검토하고, 필요 시 제거를 보류하고 사용자 판단에 맡긴다
- 사용자가 사전에 "확인 없이 진행해도 된다"며 승인을 생략하도록 요청한 경우에도, **최초 1회는 반드시 삭제 대상 전체 목록을 제시**하고 그 응답을 승인으로 갈음한다. 이후 배치까지 재확인을 생략할지는 그 시점에 다시 사용자에게 묻는다 (근거: 프로젝트 CLAUDE.md - 대상 파일 경로·행위 명시 및 명시적 승인이 실행의 전제 조건).
...
- 동적 import 확인 (문자열 패턴 grep):
  ```bash
  # ES6 dynamic import (POSIX 문자클래스 + -E: GNU/BSD grep 모두 호환)
  grep -rnE 'import[[:space:]]*\(' src/
...
### 3. 안전한 제거 프로세스
```
a) 위험도 낮은 카테고리부터 순서대로, 한 번에 한 카테고리씩 진행:
   1. 사용되지 않는 npm 종속성
   2. 사용되지 않는 내부 export
   3. 사용되지 않는 파일
   4. 중복 코드
b) 각 카테고리 실행 전, 삭제 대상 목록(경로·항목명, 위험도 표기)을 사용자에게 제시하고 명시적 승인을 받는다 - 승인 없이는 어떤 항목도 삭제하지 않는다
c) 승인된 항목만 제거
d) 각 배치 후 테스트 실행
e) 각 배치마다 git commit 생성
```
...
### 5. 코드 리뷰 (필수, 예외 없음)
code-reviewer는 에이전트가 아닌 스킬이며 이 에이전트는 Agent 도구를 갖고 있지 않으므로 직접 호출할 수 없다. 각 배치 완료 후 오케스트레이터에게 다음을 반환하여 code-reviewer 스킬 실행을 요청한다:
- 삭제/수정된 파일 목록
- 의도치 않은 로직 변경 여부 확인 요청
- CRITICAL/HIGH 이슈 발견 시 해당 배치 revert 후 재작업이 필요함을 명시
...
- [ ] 모든 테스트 실행
- [ ] 백업 브랜치 생성
- [ ] 삭제 대상 목록(위험도 표기 포함) 제시 후 사용자 명시적 승인 획득 - 등급 무관 필수, 승인 없이 삭제 실행 금지
...
```
SAFE_CANDIDATES 확정은 보호 목록 필터링 결과일 뿐 삭제 승인이 아니다 - 실제 삭제는 위 승인 게이트(대상 목록 제시 + 사용자 명시적 승인)를 통과한 뒤에만 실행한다.
...
앞선 워크플로우(카테고리별 진행, 배치별 테스트/commit, 삭제 내역 문서화)에서 다루지 않은 항목만 추가로 기억한다:
- **보수적으로** - 확실하지 않으면 제거하지 않기
- **브랜치 보호 + 동료 검토** - 항상 feature 브랜치에서 작업하고, 병합 전 삭제 내역 검토 받기
- **프로덕션 모니터링** - 배포 후 에러 주시
