...
> **취약점 하나를 찾으면, 그 "종류(클래스)"를 코드베이스 전체에서 전수로 훑는다.** 인스턴스 1건만 보고하고 끝내지 마라 - 같은 패턴(IDOR·인젝션·가드 누락·하드코딩 비밀키·입력검증 누락·경쟁조건 등)은 여러 파일에 반복되므로, 한 건 발견 즉시 그 패턴으로 grep·스캔해 **모든 발생지를 한 번에** 보고한다. (매 리뷰마다 같은 종류가 한 건씩 찔끔 나오는 것을 방지)

절차: (1) 결함 발견 → (2) "어떤 클래스인가" 정의 → (3) 그 패턴 grep으로 관련 코드 전반(라우트·핸들러·서비스·데이터 접근 계층 등) 전수 스캔 → (4) 발생지 전부 목록화(파일:라인) → (5) 각각 실취약/정상 판정 → (6) 마무리 전 자문: **"각 findings가 클래스인가? 그 클래스의 모든 인스턴스를 찾았나?"** - "아직 전수 안 함"이면 완료하지 말 것.
- 아래 §취약점 패턴 탐지의 grep 명령들을 **'한 건 확인용'이 아니라 '전수 열거용'**으로 사용한다(첫 매치에서 멈추지 말고 전체 매치를 판정).
- 보고서의 각 이슈에 **'전수 스캔 범위·발견 수'**를 명시한다(예: "IDOR: 소유자 리소스 컨트롤러 9곳 스캔 → 2곳 위반", "하드코딩 비밀키: 전체 소스 grep → 0건").
...
### 도메인 한정 고위험 패턴 (해당 도메인일 때만)

프로젝트에 아래 요소가 **있을 때만** `.claude/agent-refs/security-domain-patterns.md` 를 읽고 해당 절을 적용한다. 없으면 열지 않는다.

- WebSocket / SSE 엔드포인트가 있다 → WebSocket/SSE 인증·Origin 검증 패턴
- 결제·외부 서비스 웹훅 수신부가 있다 → 웹훅 서명 검증(타이밍 세이프 비교, 재전송 방어)
- 잔액·포인트·재고 등 금액성 자원을 변경한다 → 금융 경쟁 조건(SELECT FOR UPDATE, 원자적 UPDATE) 및 동시성 탐지 명령어
