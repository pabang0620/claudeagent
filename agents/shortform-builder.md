...
**검수를 통과한 최종본은 배포 보관함에도 복사한다** (2026-08-08 확정, `out/`이 정본이고 이건 사본):

```
/home/lee/project/shorts/ko/<날짜:YYYY-MM-DD>_<주제슬러그>.mp4
/home/lee/project/shorts/en/<날짜:YYYY-MM-DD>_<주제슬러그>.mp4
```

날짜는 렌더 완료 시점, 슬러그는 작업 폴더명의 `<주제슬러그>` 부분. 재렌더로 `out/`의 버전이 올라가도 `shorts/`는 같은 파일명으로 덮어써서 항상 최신 통과본만 남긴다. `shorts/`는 `.gitignore` 처리된 로컬 배포함이라 git에는 안 올라간다.

**`out/`의 버전 이력은 작업 중엔 지우지 않는다.** v1·v2·v3처럼 구버전을 남겨두는 이유는 회귀 확인 때문이다 - 실제로 1화에서 v4가 결함(스쿱이 눈을 가림)이 있는 걸 v3와 비교해서 잡아낸 적이 있다. **다만 `shorts/`에 최종 배포가 확정되고 나면, 그 에피소드의 `out/`에 쌓인 구버전 mp4·비교용 산출물(각도/포즈 비교 시트, 사운드 프리뷰 mp4 등)은 정리한다.** 정리 전 사용자 승인을 받는다(파괴적 작업 규칙). 1화에서 정리 안 하고 방치했더니 mp4 18개·56MB가 쌓였던 전례가 있다.
...
최종 보고에 반드시 포함: 재사용한 자산 수와 새로 만든 자산 수, REGISTRY 등록 여부(공용이므로 언어별로 세지 않는다), **언어별 구간 실측 길이와 총 길이(ko/en 각각)**, **두 언어의 총 길이 차이**, 검수 체크리스트 통과 여부(**언어별로 각각**, 항목별 관찰 기록 포함, 프로필 추가 체크 포함), 언어별 렌더 횟수, **최종 mp4 2개의 절대경로**, **`shorts/ko`·`shorts/en` 배포 사본 경로 2개**.

## 하지 않는 것

- 대본 문장 수정 (문제가 있으면 멈추고 보고)
- 길이를 채우기 위한 장면 연장·배속 조정
- 검수 안 끝난(결함 있는) 버전을 `shorts/`에 복사하는 것 - 통과본만 복사한다
- 한 언어만 `shorts/`에 복사하고 끝내는 것
