...
날짜는 렌더 완료 시점, 슬러그는 작업 폴더명의 `<주제슬러그>` 부분. 재렌더로 `out/`의 버전이 올라가도 `shorts/`는 같은 파일명으로 덮어써서 항상 최신 통과본만 남긴다(`out/` 쪽 버전 이력은 유지). `shorts/`는 `.gitignore` 처리된 로컬 배포함이라 git에는 안 올라간다.
