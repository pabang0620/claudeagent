...
# 기존 디렉토리 체크 (Vite CLI 충돌 방지)
for dir in frontend backend; do
  if [ -d "$dir" ]; then
    echo "[WARN] '$dir/' 폴더가 이미 존재합니다. 재초기화 후 실행하세요: rm -rf $dir"
    exit 1
  fi
done
...
## Phase 1.5: 입력 수집 (Bash 도구 사용 금지 — 사용자 메시지 교환)

아래 질문을 사용자에게 직접 출력하고 응답을 기다린다:
---
Bootstrap을 시작합니다. 설정을 확인해주세요:
1. TypeScript 사용 여부 (기본값: yes) [yes/no]:
2. S3 파일 업로드 사용 여부 (기본값: yes) [yes/no]:
3. 프로젝트 이름 (기본값: 현재 디렉토리명):
4. 추가 도메인 목록 (user + auth 외, 쉼표 구분, 기본값: 없음):
---
사용자 응답을 변수로 저장:
- USE_TYPESCRIPT = (사용자 응답, 미응답 시 "true")
- USE_S3 = (사용자 응답, 미응답 시 "true")
- USER_PROJECT_NAME = (사용자 응답, 미응답 시 빈 값 → Phase 2에서 디렉토리명으로 대체)
- EXTRA_DOMAINS = (사용자 응답, 미응답 시 빈 값 → user + auth 만 생성)

위 4개 변수 외에 Phase 2~5 에서 참조하는 사용자 입력 변수가 추가로 있으면 여기서 함께 수집한 뒤 사용한다.

이후 Phase 2부터 Bash 도구 실행.

## Phase 1 마지막: 생성 파일 목록 출력 및 승인

사용자에게 다음 메시지를 출력하고 응답을 기다린다:
---
다음 파일들을 생성합니다 (총 27~30개):
[인프라] .env.example, backend/config/env.js, backend/app.js
[선택: S3=true] backend/utils/s3.js
[ui-design-system] frontend/src/styles/tokens.css + 공용 컴포넌트 13종
[테스트] frontend/src/mocks/, playwright.config.ts
[CI/CD] .github/workflows/ci.yml, .husky/pre-commit

계속 진행하시겠습니까? [yes/no] (no 입력 시 중단):
---
"no" 응답 시 즉시 중단.
...
npm install express cors zod jsonwebtoken bcryptjs mysql2 multer @aws-sdk/client-s3 dotenv glob
npm install -D nodemon jest supertest
# backend package.json 에 scripts 추가
npm pkg set scripts.dev="nodemon app.js" scripts.start="node app.js" scripts.test="jest --passWithNoTests"
cd ..
...
EOF
# husky는 .git 필요 — 없으면 먼저 git init
[ -d .git ] || git init
npm i -D husky@^9
npx husky init   # .husky/ 디렉토리 + 기본 pre-commit 생성 (stdin 불필요, 자동 실행됨)
# git init && npx husky init 후 .husky/pre-commit를 덮어쓴다 (husky v9도 init 필요)
...
# JWT
JWT_SECRET=devsecret_devsecret_devsecret_32x   # 32자 이상
JWT_EXPIRES_IN=7d
...
try {
  ...
  console.error(JSON.stringify(e.flatten?.()?.fieldErrors ?? e.message, null, 2))
  process.exit(1)
}
...
// 임시 에러 핸들러 (Phase 4 이전 사용)
app.use((err, req, res, next) => {
  res.status(err.status || 500).json({ success: false, error: err.message || '서버 오류' })
})
...
> **TypeScript 분기 (USE_TYPESCRIPT 값에 따라)**:
> - USE_TYPESCRIPT=true → TypeScript 버전 생성 (기본)
> - USE_TYPESCRIPT=false → JavaScript 버전 생성 (tseslint 제거, tsc 제거)

**USE_TYPESCRIPT=true 일 때:**
```javascript
...
**USE_TYPESCRIPT=false 일 때 생성하는 버전 (TypeScript 플러그인 없는 버전):**
```javascript
// USE_TYPESCRIPT=false일 때 생성하는 버전 (TypeScript 플러그인 없는 버전)
import js from '@eslint/js'
import globals from 'globals'
import reactHooks from 'eslint-plugin-react-hooks'
import reactRefresh from 'eslint-plugin-react-refresh'
...
# 3. 타입 체크 (tsconfig.json 존재 시에만 실행 — JS 프로젝트에서 자동 스킵)
cd frontend && { [ -f tsconfig.json ] && npx tsc --noEmit || echo "SKIP: tsconfig.json 없음 (JS 프로젝트)"; } && cd ..
...
jobs:
  build:
    ...
    steps:
      ...
      # tsconfig.json 존재 시에만 실행 — JS 프로젝트에서 자동 스킵
      - name: Type Check
        run: |
          cd frontend && { [ -f tsconfig.json ] && npm run typecheck || echo "SKIP: tsconfig.json 없음 (JS 프로젝트)"; }
      - name: Test
        run: |
...
cd backend && { [ -f .env ] || cp ../.env.example .env; } && \
  node -e "import('./config/env.js').then(()=>console.log('env OK')).catch(e=>{console.error(e);process.exit(1)})" ; cd ..
