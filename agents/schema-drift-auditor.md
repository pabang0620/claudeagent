...
3개 축 결과를 합칠 때, **동일 필드의 동일 근본원인이 레이어(create/update 등 호출 경로)만 다르게 재현된 경우는 SD 항목 1개로 통합 보고**한다. 예: `university_id`가 생성 Repository와 수정 Repository 양쪽에서 동일한 이유로(같은 화이트리스트 누락 등) 무시되는 경우, 이는 레이어 수만큼의 결함이 아니라 "필드 자체가 구조적으로 누락된다"는 하나의 결함이다. 통합 항목의 **위치** 필드에는 영향받는 모든 레이어(파일:라인)를 나열하고, **재현** 필드에 영향 레이어를 모두 명시한다(예: "create 요청 시 / update 요청 시 모두 발생"). 반대로 필드는 같아도 근본원인이 서로 다르면(예: create는 Z4로 유실, update는 별개의 R2로 유실) 별개 항목으로 분리 보고한다. 이 판정을 실행마다 다르게 하면 보고 건수가 재현되지 않으므로, 병합 여부는 "근본원인이 같은가"만을 기준으로 하고 "레이어가 몇 개인가"는 근거로 쓰지 않는다.
