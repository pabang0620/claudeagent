...
| 프로젝트 | 라우트 검출 | 테이블 매핑 | 화면 매핑 |
|---|---|---|---|
| wecom | 137 / 142 (96%) | 71% | 85% |
| modadam | 29 / 29 (100%) | 76% | 34% |
| boothflow | 49 / 52 (94%) | 76% | 82% |
| cosmic-renew | 119 / 119 (100%) | 78% | 74% |

## 알려진 한계

정확도보다 **틀린 값을 안 만드는 것**을 우선했다. 아래는 의도된 빈칸이다.

- **테이블 매핑 71~78%.** 같은 함수명이 여러 도메인에 있고 어느 쪽인지 확정할 수
  없으면 비워 둔다(boothflow 실측 동명이인 60건). 추측해서 채우면 지도가 거짓말을 한다.
- **동적 경로.** 변수로 조립되는 라우트는 `unresolved` 로 남긴다.
- **동적 호출.** 프론트에서 변수로 URL을 만들면 화면↔API 연결이 끊긴다.
  modadam 화면 매핑이 34%로 낮은 것이 이 경우다.
- **데이터 예시는 생성한 값**이다. 실제 응답이 아니라 Zod 스키마·DB 컬럼에서
  만든 것이고, 모달마다 출처를 표기한다.
- **점검 항목은 신호이지 판정이 아니다.** 고아 API는 동적 호출을 못 잡은 것일 수 있다.
