...
- 표의 **근거ID**는 Evidence 파일 저장 여부와 무관하게 `E-[주제슬러그]-[2자리 일련번호]` 형식으로 부여한다 (예: `E-contents2026-01`).

---

## 근거자료(Evidence) 파일 저장 — 선택, 조건부 실행

다음 중 하나에 해당할 때만 실행한다 (기본 조사 흐름에는 영향 없음):
- 사용자가 "근거자료로 저장", "evidence 파일로", "PT/문서에 쓸 수 있게 저장" 등을 명시적으로 요청
- 호출자가 proposal-pt-builder·doc-generator 등 문서작성 연계 목적으로 이 스킬을 호출

### 실행 절차
1. 위 "핵심 수치 및 근거" 표의 각 행에 부여한 **근거ID**를 그대로 재사용한다. 컬럼 매핑: `내용`+`수치/사실` → `핵심 내용/수치`로 병합, `연도` → `발행일`, `활용포인트`는 "PPT 활용 포인트" 내용을 행별로 요약해 채운다.
2. 저장 경로가 요청에 명시되어 있으면 그 경로를 사용한다. 없으면 대화로 "근거자료를 저장할 폴더 절대경로를 알려주세요"라고 1회 질문한다.
3. 파일명은 `evidence_<주제슬러그>.md` 로 정한다 (단발 심층 리서치이므로 타겟슬러그 없이 주제슬러그만 사용).
4. **Write 도구**로 아래 표준 스키마에 맞춰 저장한다.
5. 결과 응답의 "PPT 활용 제안" 섹션 아래에 저장한 파일의 절대경로를 한 줄로 보고한다.
...
```
- **근거ID**: `E-[주제슬러그]-[2자리 일련번호]`, 파일 내 중복 금지
